    ...
    >
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          ...
          >
            <Timer className={`h-4 w-4 ${sidebarCollapsed && !isMobileSidebar ? '' : 'mr-2'}`} />
            {(!sidebarCollapsed || isMobileSidebar) && (
              <>
                <span>{langs[lang].components.rightSidebar.pending}</span>
                <Badge className="ml-auto">
                  {
                    filteredNotifications.filter((n) => !(n.metadata?.taskCompleted ?? false))
                      .length
                  }
                </Badge>
              </>
          ...
          >
            <Clock className={`h-4 w-4 ${sidebarCollapsed && !isMobileSidebar ? '' : 'mr-2'}`} />
            {(!sidebarCollapsed || isMobileSidebar) && (
