...
export function PrescriptionsProvider({
  ...
  const [prescriptions, setPrescriptions] = useState<PrescriptionTableData[]>(initialPrescriptions)
  const [selectedPrescription, setSelectedPrescription] = useState<PrescriptionTableData | null>(
    null
  )
  ...
  const performDialogAction = async (action: 'cancel' | 'renew') => {
    ...
    try {
      ...
      console.log(`[Prescriptions] ${action} prescription ${selectedPrescription.prescriptionId}`)
    } catch (error) {
      const message =
