...
export function PatientDetailsDialog() {
  ...
  const fullName = detail ? `${detail.firstName} ${detail.lastName}`.trim() : selectedPatient.name
  ...
  const medicalCenter = detail?.mainAffiliationName ?? dictionary.medicalCenterFallback
        ...
        <div className="space-y-6 py-2">
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-3 text-sm md:grid-cols-2 md:gap-5">
              <InfoRow
                label={dictionary.ageLabel}
                value={age != null ? `${age}` : dictionary.notAvailable}
              />
...
function mapGender(
  value: number,
  options: { male: string; female: string; other: string }
): string {
  switch (value) {
    case 0:
      return options.male
